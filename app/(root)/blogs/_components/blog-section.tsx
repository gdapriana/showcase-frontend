  ...
  return (
    <div className="w-full md:w-9/12 gap-4 md:flex-1 md:grow md:basis-0 md:overflow-auto flex flex-col justify-start items-stretch">
      {blogs.map((blog: Blog, index: number) => {
        return (
          <Link href={`/blogs/${blog.slug}`} key={index} className="border relative gap-4 p-8 flex flex-col justify-start items-stretch">
            <m.div 
              animate={{ width: ["100%", "2%"] }}
              transition={{ duration: 2, ease: "anticipate", delay: index * 0.2 }}
              className="absolute top-0 left-0 bg-primary h-full"></m.div>
